...
export function analyzeReading({ storyText, transcript, durationSeconds, recognitionConfidence = 0.6, previousBest = 0 }) {
  ...
  const scoreRaw = (
    (accuracy * 0.25) +          // Reduced from 0.28
    (fluency * 0.25) +           // Increased from 0.2
    (speed * 0.12) +             // Reduced from 0.14
    (pauseDensity * 0.12) +      // Reduced from 0.14  
    (expression * 0.16) +        // Increased from 0.14
    (improvement * 0.1)          // Same
  ) / 10;
  
  const score = Math.max(1, Math.min(10, Number(scoreRaw.toFixed(1))));
  
  // More encouraging messages for children
  const message = score >= 8.5
