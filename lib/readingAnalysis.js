...
export function analyzeReading({ storyText, transcript, durationSeconds, recognitionConfidence = 0.6, previousBest = 0 }) {
  ...
  // Visual star display
  const starDisplay = 'â­'.repeat(starRating) + (hasHalfStar ? 'âœ¨' : '');
  ...
  if (numericScore >= 9) {
    emojiScore = 'ðŸŒŸðŸŽ‰âœ¨'; // Super star!
    category = 'Reading Rockstar!';
  } else if (numericScore >= 8) {
    emojiScore = 'ðŸŽ‰ðŸ˜ŠðŸ‘'; // Excellent!
    category = 'Excellent Reader!';
  } else if (numericScore >= 7) {
    emojiScore = 'ðŸ˜ŠðŸ‘ðŸ‘'; // Great job!
    category = 'Great Job!';
  } else if (numericScore >= 6) {
    emojiScore = 'ðŸ‘ðŸ‘ðŸ’ª'; // Good work!
    category = 'Good Work!';
  } else if (numericScore >= 5) {
    emojiScore = 'ðŸ‘ðŸ’ªðŸ“š'; // Keep going!
    category = 'Keep Practicing!';
  } else {
    emojiScore = 'ðŸ’ªðŸ“šðŸŒ±'; // You're learning!
    category = 'You\'re Learning!';
  }

  // More encouraging messages for children
  const message = numericScore >= 8.5
    ? 'ðŸŒŸ Amazing reading! You read with great expression and confidence!'
    : numericScore >= 7
      ? 'ðŸŽ‰ Wonderful job! Your reading is getting better every time!'
      : numericScore >= 5.5
        ? 'ðŸ‘ Good work! You finished the story and practiced your reading!'
        : 'ðŸ’ª You completed the story! Every time you read, you get better!';
