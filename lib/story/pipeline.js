...
export function computeStoryTargets({ rwLevel = 'blue', previousStoryWordCount, recentPerformance, engagement } = {}) {
  ...
  const minimumWords = Math.max(300, Math.round(targetWords * 0.9));
  const preferredPageWordMin = targetWords <= 400 ? 65 : targetWords < 700 ? 80 : 95;
  const preferredPageWordMax = targetWords <= 400 ? 110 : targetWords < 1000 ? 140 : 160;
  const targetPageCountMin = targetWords <= 400 ? 4 : targetWords < 800 ? 6 : 7;
  const targetPageCountMax = targetWords <= 400 ? 6 : targetWords < 800 ? 9 : 12;
