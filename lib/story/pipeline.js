const DEFAULT_MINIMUM_WORD_TARGET = 300;
...
export function computeStoryTargets({ rwLevel = 'turquoise', previousStoryWordCount, recentPerformance, engagement } = {}) {
  const baseByLevel = {
    red: 260,
    pink: 280,
    purple: 300,
  ...
  const baseTarget = baseByLevel[rwLevel] || baseByLevel.turquoise;
  ...
  targetWords = Math.max(260, Math.min(1500, Math.round(targetWords)));

  const minimumWords = Math.max(240, Math.round(targetWords * 0.85));
  const preferredPageWordMin = targetWords <= 400 ? 45 : targetWords < 700 ? 55 : 65;
  const preferredPageWordMax = targetWords <= 400 ? 80 : targetWords < 1000 ? 100 : 110;
  const targetPageCountMin = targetWords <= 400 ? 5 : targetWords < 800 ? 7 : 8; // More pages, less words per page
  const targetPageCountMax = targetWords <= 400 ? 7 : targetWords < 800 ? 10 : 14;
...
export function splitStoryIntoPages(storyText, { minWords = 45, maxWords = 80 } = {}) {
