...
export function getTargetPageCount(readingSessions = []) {
  ...
  if (avgScore >= 8.5) return 8;
  if (avgScore >= 7.5) return 7;
  if (avgScore >= 6.5) return 6;
  return 5; // Default for new users or lower scores
}
...
export function updateDifficultyProfile(child, readingSessions = []) {
  ...
  if (child.currentLevel === 'turquoise' && nextProgress > 0.95 && avgAccuracy > 85) {
    return { ...next, currentLevel: 'blue', nextMilestone: 'blue' };
  }
